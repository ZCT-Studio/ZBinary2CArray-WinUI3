...
public:
    ...
    void Reload() {
        if (!ZBTCA_Details::fs::exists(m_path))
            throw std::invalid_argument("File not found: " + ZBTCA_Details::path_to_utf8(m_path));
        if (!ZBTCA_Details::fs::is_regular_file(m_path))
            throw std::invalid_argument("Path is not a regular file: " + ZBTCA_Details::path_to_utf8(m_path));
        std::ifstream ifs(m_path, std::ios::binary);
        if (!ifs) throw std::runtime_error("Failed to open file: " + ZBTCA_Details::path_to_utf8(m_path));
        ...
        if (!ifs) throw std::runtime_error("Failed to read file or incomplete read: " + ZBTCA_Details::path_to_utf8(m_path));
    }
    ...
    [[nodiscard]] std::string GetFilename() const { return ZBTCA_Details::path_to_utf8(m_path.filename()); }
